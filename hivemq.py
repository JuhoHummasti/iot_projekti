...
# mqtt client connect
client = MQTTClient(client_id=b'Laki', server=config.MQTT_BROKER, port=config.MQTT_PORT,
...
i2c = machine.I2C(id=0, sda=Pin(0), scl=Pin(1)) # id=channel
bmp = BMP280(i2c)
...
while True:
    ...
    publish(client, 'Laki/temp:', str(bmp.temperature))
    publish(client, 'Laki/pressure:', str(bmp.pressure))
